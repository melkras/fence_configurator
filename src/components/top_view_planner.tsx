...
export default function TopViewPlanner() {
    ...
    const handleClick = (e: any) => {
        ...
        if (angleLock && points.length > 0) {
            ...
            [snappedX, snappedZ] = getSnappedAngle(last, [snappedX, snappedZ], angleStepRad);
        }
        ...
        if (points.length > 1) {
            ...
            if (distance < gridSize * 0.5) { // Threshold to consider it connected
                setCompletedDrawings([...completedDrawings, [...points, [startX, startZ]]]); // Save the completed drawing including the last line
    ...
    const handleWheel = (e: WheelEvent) => {
        ...
        if (cameraRef.current) {
            cameraRef.current.zoom = Math.max(10, Math.min(100, cameraRef.current.zoom - e.deltaY * 0.01));
        ...
        >
            ...
            {completedDrawings.map((drawing, index) => (
                drawing.map((pt, i) => {
                    ...
                    return <Line key={`${index}-${i}`} points={[start, end]} color="blue" lineWidth={2} />;
                })
            ))}
            ...
            {points.length >= 2 &&
                points.map((pt, i) => {
                    ...
                    const end = new Vector3(pt[0], 0.02, pt[1]);
                    return <Line key={i} points={[start, end]} color="black" lineWidth={2} />;
                })}
            {/* Preview segment */}
            {preview && points.length > 0 && (
                <Line
                    points={[
                        new Vector3(points[points.length - 1][0], 0.02, points[points.length - 1][1]),
